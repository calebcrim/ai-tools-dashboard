...
class AIToolExtractor {
    constructor() {
        ...
        this.toolPatterns = {
            launch: /(?:launches?|unveiled?|introduces?|announced?|released?)\s+(?:its\s+)?(?:new\s+)?([A-Z][A-Za-z0-9\s\-\.]+?)(?:\s+AI|\s+platform|\s+tool)/gi,
            update: /([A-Z][A-Za-z0-9\s\-\.]+?)\s+(?:adds?|updates?|upgrades?|enhances?|improves?|expands?)/gi,
            acquisition: /([A-Z][A-Za-z0-9\s\-\.]+?)\s+(?:acquires?|acquired by|buys?|purchased?)/gi,
            funding: /([A-Z][A-Za-z0-9\s\-\.]+?)\s+(?:raises?|raised|secures?|secured|funding|investment)/gi,
            partnership: /([A-Z][A-Za-z0-9\s\-\.]+?)\s+(?:partners?|partnership|collaborates?|collaboration|integrates?)/gi,
    ...
    async loadExistingTools() {
        try {
            ...
            console.log(`Loaded ${data.tools.length} existing tools`);
        } catch (error) {
    ...
    async generateToolEntry(extractedTool) {
        ...
        return {
            ...
            brief_purpose_summary: `${extractedTool.name} - ${extractedTool.context.substring(0, 150)}...`,
...
module.exports = AIToolExtractor;